...
class ReferenceDataServiceTest {
    ...
    void getAllDepartments_Empty() {
        when(config.getDepartments()).thenReturn(null);
        StepVerifier.create(service.getAllDepartments())
                .verifyComplete(); // Mono.justOrEmpty(null) results in an empty Mono
    }
    ...
    @Test
    void validateDepartmentOnly_Success() {
        when(config.isValidDepartment("DEPT_01")).thenReturn(true);
        StepVerifier.create(service.validateDepartmentOnly("DEPT_01"))
                .verifyComplete();
    }

    @Test
    void validateDepartmentOnly_Invalid() {
        when(config.isValidDepartment("INVALID_DEPT")).thenReturn(false);
        StepVerifier.create(service.validateDepartmentOnly("INVALID_DEPT"))
                .expectError(IllegalArgumentException.class)
                .verify();
    }
}