...
class ReferenceDataServiceTest {
	...
	void getAllDepartments_Success() {
		...
		StepVerifier.create(service.getAllDepartments()).expectNextMatches(map -> map.containsKey("DEPT_01"))
				.verifyComplete();
	}
	...
	void getCategoriesByDepartment_Success() {
		...
		StepVerifier.create(service.getCategoriesByDepartment("DEPT_01"))
				.expectNextMatches(map -> map.containsKey("CAT_01")).verifyComplete();
	}

	@Test
	void getCategoriesByDepartment_NotFound() {
		when(config.isValidDepartment("DEPT_01")).thenReturn(false);
		StepVerifier.create(service.getCategoriesByDepartment("DEPT_01")).expectError(ResponseStatusException.class)
				.verify();
	}
	...
	void validateDepartmentAndCategory_Success() {
		...
		StepVerifier.create(service.validateDepartmentAndCategory("DEPT_01", "CAT_01")).verifyComplete();
	}
	...
	void getSlaHours_Success() {
		...
		StepVerifier.create(service.getSlaHours("DEPT_01", "CAT_01")).expectNext(48).verifyComplete();
	}
}
