...
class UserControllerTest {
        ...
        void updateProfile_success() {
                ...
                when(userService.updateProfile(
                                ...
                                any(),
                                eq("u1"),
                                eq(ROLE.CITIZEN))).thenReturn(Mono.just(
        ...
        void updateProfile_forbidden() {
                ...
                when(userService.updateProfile(
                                ...
                                any(),
                                eq("u2"),
                                eq(ROLE.CITIZEN))).thenReturn(Mono.error(
        ...
        void updateUserRole_adminOnly_success() {
                when(userService.updateRole(
                                ...
                                eq("OFFICER"),
                                eq("admin"),
                                eq(ROLE.ADMIN))).thenReturn(Mono.empty());
        ...
        void updateUserRole_forbidden() {
                when(userService.updateRole(
                                any(), any(), any(), eq(ROLE.CITIZEN))).thenReturn(Mono.error(
        ...
        void assignDepartment_success() {
                when(userService.updateDepartment(eq("u2"), eq("D1"), eq("admin"), eq(ROLE.ADMIN)))
                                .thenReturn(Mono.empty());

                webTestClient.put()
                                .uri("/users/u2/department/D1")
                                .header("X-USER-ID", "admin")
                                .header("X-USER-ROLE", "ADMIN")
                                .exchange()
                                .expectStatus().isOk();
        ...
        void getOfficersByDepartment_supervisorSuccess() {
                when(userService.getOfficersByDepartment(eq("D1"), eq("s1"), eq(ROLE.SUPERVISOR)))
                                .thenReturn(Flux.just(UserResponse.builder().id("o1").build()));
        ...
        @Test
        void getSupervisorByDepartment_success() {
                when(userRepository.findByRoleAndDepartmentId(ROLE.SUPERVISOR, "D1"))
                                .thenReturn(Flux.just(com.egov.user.model.User.builder().id("s1").build()));

                webTestClient.get()
                                .uri("/users/supervisor/department/D1")
                                .exchange()
                                .expectStatus().isOk()
                                .expectBody(String.class).isEqualTo("s1");
        }

        @Test
        void getSupervisorByDepartment_notFound() {
                when(userRepository.findByRoleAndDepartmentId(ROLE.SUPERVISOR, "D1"))
